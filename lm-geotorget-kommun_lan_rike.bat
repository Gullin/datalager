...
SET DL_PROCESSNAME=lm-geotorget-kommun_lan_rike
REM Processlokala parametrar
SET DL_OUTDIR=lantmateriet\kommun_lan_rike
SET DL_OUTDBSCHEMA=data_auto_lantmateriet_kommun_lan_rike
SET DL_FMEPROCESS01="%DL_PROCESSNAME%\_fme\01-download-geotorget-http_basic_auth.fmw"
SET DL_FMEPROCESS02="%DL_PROCESSNAME%\_fme\02-geotorget-kommun_lan_rike-DatalagerManage-driver.fmw"
...
IF %ERRORLEVEL% EQU 0 (
    ...
    IF %ERRORLEVEL% EQU 0 (
        ...
        @CALL _sys\_schema-driver %DL_PROCESSNAME% %DL_ISWHOLEPROCESS% validate NULL true OGCGEOPACKAGE
        ...
        @CALL _sys\_log-batch ERROR "Processen %DL_PROCESSID% kunde ladda ner data"
        @CALL _sys\_log-error %DL_PROCESSID% "Errorlevel %ERRORLEVEL% f”r %DL_PROCESSID%, kunde inte ladda ner data" %DL_PROCESSNAME%
...
:DownloadData
    ...
    >nul (
        @%DL_FMEFULLPATH% %DL_FMEPROCESS01% ^
                            ...
                            --USERNAME %LM-USER-LAND0003% ^
                            --PASSWORD %LM-PASS-LAND0003% ^
